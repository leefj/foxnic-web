...
public class FoxnicWebEnumGenerator  {
	...
	private void buildEnums() {
		...
		//系统配置 sys_config
		EnumConfig info=new EnumConfig(SYS_CONFIG.CODE,SYS_CONFIG.NAME , new ConditionExpr("profile_id='default' and type!='DIR' and deleted=0 and (code like 'system.%' or code like 'modules%')"));
...
class DictItemBuilder extends JavaClassFile {
	...
	protected void buildBody() {
		...
		for (Rcd r : items) {
			String codeValue=r.getString(SYS_DICT_ITEM.CODE);
			String name=codeValue;
			name=name.replace('.', '_');
