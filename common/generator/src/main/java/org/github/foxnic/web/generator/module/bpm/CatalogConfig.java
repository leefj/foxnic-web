...
public class CatalogConfig extends BaseCodeConfig<BPM_CATALOG> {
    ...
    public CatalogConfig() {
        super(PREFIX_BPM, BPM_CATALOG.$TABLE,"bpm_", 4);
    }
...
//        view.form().addCss("/form/demo/demo.css");
    ...
    public void configFields(ViewOptions view) {
        ...
        view.field(BPM_CATALOG.ID).basic().hidden();
...
//                .form().validate().required();
        view.field(BPM_CATALOG.VALID).search().hidden().form().logicField().on("有效",1).off("无效",0).defaultValue(true);
