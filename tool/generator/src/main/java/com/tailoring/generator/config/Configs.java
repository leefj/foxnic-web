...
public class Configs {
	...
	public Configs() {
		this(ProjectConstants.SERVICE_TAILORING_PROJECT_FOLDER_NAME, ProjectConstants.PRIMARY_DATASOURCE_CONFIG_KEY, ProjectConstants.NACOS_GROUP, ProjectConstants.DEFAULT_NACOS_DATA_ID);
	}
	
	public Configs(String serviceProjectFolderName) {
		this(serviceProjectFolderName, ProjectConstants.PRIMARY_DATASOURCE_CONFIG_KEY, ProjectConstants.NACOS_GROUP, ProjectConstants.DEFAULT_NACOS_DATA_ID);
	}
	
	public Configs(String serviceProjectFolderName,String datasourceConfigKey,String nacosGroup,String nacosDataId) {
		
		initGlobalSettings();
 
		//
		generatorProject=new MavenProject(Configs.class);
		//
		File domainProjectFolder=FileUtil.resolveByPath(generatorProject.getProjectDir().getParentFile(), ProjectConstants.DOMAIN_PROJECT_FOLDER_NAME);
		domianProject=new MavenProject(domainProjectFolder); 
		
		File serviceProjectFolder=FileUtil.resolveByPath(generatorProject.getProjectDir().getParentFile(), serviceProjectFolderName);
		serviceProject=new MavenProject(serviceProjectFolder);
		
		File proxyProjectFolder=FileUtil.resolveByPath(generatorProject.getProjectDir().getParentFile(), ProjectConstants.AGENT_PROJECT_FOLDER_NAME);
	...
	private void initGlobalSettings() {
		...
		this.settings.setEnableMicroService(ProjectConstants.ENABLE_MICRO_SERVICE);
