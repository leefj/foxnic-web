...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('example-order-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('example-order-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'example-order-form-data-win');
			if(area==null) return;
			admin.putTempData('example-order-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("goodsNames",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('example-order-form-data');
		}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('example-order-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('example-order-form-data-win',this); });
