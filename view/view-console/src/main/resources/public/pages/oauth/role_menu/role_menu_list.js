...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-oauth/sys-role-menu";
     ...
     function renderTable() {
		...
		fox.renderTable({
			...
			cols: [[
                ...
                { field: 'menuId', sort: true, title: fox.translate('菜单ID') } ,
	...
	function bindButtonEvent() {
        ...
        $('#delete-button').click(function () {
            ...
            if(ids.length==0) {
            	layer.msg(fox.translate('请选择需要删除的')+fox.translate('角色账户关系')+"!");
            	return;
            }
            //调用批量删除接口
			layer.confirm(fox.translate('确定删除已选中的')+fox.translate('角色账户关系')+fox.translate('吗？'), function (i) {
				layer.close(i);
				layer.load(2);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			if (layEvent === 'edit') { // 修改
				...
				layer.confirm(fox.translate('确定删除此')+fox.translate('角色账户关系')+fox.translate('吗？'), function (i) {
					layer.close(i);
					layer.load(2);
	...
	function showEditForm(data) {
		...
		admin.putTempData('sys-role-menu-form-data', data);
		var area=admin.getTempData('sys-role-menu-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= ($(window).height()-height)/2;
		var title = (data && data.id) ? (fox.translate('修改')+fox.translate('角色账户关系')) : (fox.translate('添加')+fox.translate('角色账户关系'));
		admin.popupCenter({
			title: title,
			resize:true,
			offset:[top,null],
			...
			type: 2,
			content: '/pages/oauth/role_menu/role_menu_form.html' + queryString,
			finish: function () {
				refreshTableData();
			}
		});
