...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('sys-sms-verification-code-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('sys-sms-verification-code-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'sys-sms-verification-code-form-data-win');
			if(area==null) return;
			admin.putTempData('sys-sms-verification-code-form-area', area);
	...
	function renderFormFields() {
		...
		laydate.render({
			...
			trigger:"click",
				...
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("expireTime",value, date, endDate);
			}
		});
	}
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('sys-sms-verification-code-form-data');
		}
		...
		if (hasData) {
			...
			if(formData["expireTime"]) {
				$("#expireTime").val(fox.dateFormat(formData["expireTime"],"yyyy-MM-dd HH:mm:ss"));
			}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('sys-sms-verification-code-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('sys-sms-verification-code-form-data-win',this); });
