...
function ListPage() {
		...
		function renderTableInternal() {
			...
			dataTable=fox.renderTable({
				...
				cols: [[
					...
					,{ field: 'orgId', align:"left",fixed:false,  hide:false, sort: true, title: fox.translate('所属组织ID') , templet: function (d) { return templet('orgId',d.orgId,d);}  }
					,{ field: 'code', align:"left",fixed:false,  hide:false, sort: true, title: fox.translate('岗位代码') , templet: function (d) { return templet('code',d.code,d);}  }
	...
	function bindButtonEvent() {
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('hrm-position-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的')+fox.translate('岗位')+"!");
            	return;
            }
            //调用批量删除接口
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('hrm-position-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				//延迟显示加载动画，避免界面闪动
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('hrm-position-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString="?" + 'id=' + data.id;
		admin.putTempData('hrm-position-form-data', data);
		var area=admin.getTempData('hrm-position-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('岗位');
		...
		var index=admin.popupCenter({
			...
			content: '/business/hrm/position/position_form.html' + queryString,
			finish: function () {
				refreshTableData();
			}
		});
		admin.putTempData('hrm-position-form-data-popup-index', index);
	};
