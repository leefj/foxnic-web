...
function ListPage() {
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('hrm-favourite-group-item-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的')+fox.translate('常用人员分组条目')+"!");
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的')+fox.translate('常用人员分组条目')+fox.translate('吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('hrm-favourite-group-item-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				admin.post(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('hrm-favourite-group-item-form-data-form-action', "edit",true);
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('hrm-favourite-group-item-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('hrm-favourite-group-item-form-data', data);
		var area=admin.getTempData('hrm-favourite-group-item-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('常用人员分组条目');
		...
		admin.popupCenter({
			...
			type: 2,
			id:"hrm-favourite-group-item-form-data-win",
			content: '/business/hrm/favourite_group_item/favourite_group_item_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
