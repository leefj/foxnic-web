...
function FormPage() {
	...
	this.init=function(layui) {
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('sys-code-example-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'sys-code-example-form-data-win');
			if(area==null) return;
	...
	function renderFormFields() {
		...
		form.on('radio(radioEnum)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=radioEnum]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("radioEnum",data,checked);
		});
		form.on('radio(radioDict)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=radioDict]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("radioDict",data,checked);
		});
		form.on('checkbox(checkEnum)', function(data){
			var checked=[];
			$('input[type=checkbox][lay-filter=checkEnum]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onCheckBoxChanged && window.pageExt.form.onCheckBoxChanged("checkEnum",data ,checked);
		});
		form.on('checkbox(checkDict)', function(data){
			var checked=[];
			$('input[type=checkbox][lay-filter=checkDict]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onCheckBoxChanged && window.pageExt.form.onCheckBoxChanged("checkDict",data ,checked);
		});
		//渲染 selectEnum 下拉字段
		fox.renderSelectBox({
			el: "selectEnum",
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("selectEnum",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("selectDict",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			transform: function(data) {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("resourceId",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			...
			transform: function(data) {
				...
				if(action=="create") {
					defaultValues = "".split(",");
					defaultIndexs = "1".split(",");
				}
				var opts=[];
				if(!data) return opts;
				for (var i = 0; i < data.length; i++) {
					if(!data[i]) continue;
					opts.push({data:data[i],name:data[i].url,value:data[i].id,selected:(defaultValues.indexOf(data[i].id)!=-1 || defaultIndexs.indexOf(""+i)!=-1)});
				}
				return opts;
			}
		});
		...
		laydate.render({
			...
			trigger:"click",
				...
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("workTime",value, date, endDate);
			}
		});
		//渲染 roleIds 下拉字段
		fox.renderSelectBox({
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('sys-code-example-form-data');
		}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('sys-code-example-form-data-win',this); });
