...
function ListPage() {
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'catalogCode', align:"left",fixed:false,  hide:true, sort: true  , title: fox.translate('分类代码'), templet:function (d){ return templet('catalogCode',fox.getDictText(SELECT_CATALOGCODE_DATA,d.catalogCode,'','catalogCode'),d);}}
					...
					,{ field: 'type', align:"left",fixed:false,  hide:true, sort: true  , title: fox.translate('数据类型'), templet:function (d){ return templet('type',fox.getEnumText(RADIO_TYPE_DATA,d.type,'','type'),d);}}
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		var value = {};
		value.catalogCode={ inputType:"select_box", value: getSelectedValue("#catalogCode","value"), label:getSelectedValue("#catalogCode","nameStr") };
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('sys-config-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'系统配置'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'系统配置'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			if (layEvent === 'edit') { // 修改
				...
				top.layer.confirm(fox.translate('确定删除此'+'系统配置'+'吗？'), function (i) {
					top.layer.close(i);
