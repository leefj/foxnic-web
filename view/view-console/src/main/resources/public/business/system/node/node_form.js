...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('sys-node-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('sys-node-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'sys-node-form-data-win');
			if(area==null) return;
	...
	function renderFormFields() {
		...
		laydate.render({
			...
			trigger:"click",
				...
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("heartBeatTime",value, date, endDate);
			}
		});
	}
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('sys-node-form-data');
		}
		...
		if (hasData) {
			...
			//设置 心跳时间 显示复选框勾选
			if(formData["heartBeatTime"]) {
				$("#heartBeatTime").val(fox.dateFormat(formData["heartBeatTime"],"yyyy-MM-dd HH:mm:ss"));
			}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('sys-node-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('sys-node-form-data-win',this); });
