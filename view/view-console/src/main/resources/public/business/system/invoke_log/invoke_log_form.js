...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('sys-invoke-log-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('sys-invoke-log-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'sys-invoke-log-form-data-win');
			if(area==null) return;
			admin.putTempData('sys-invoke-log-form-area', area);
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('sys-invoke-log-form-data');
		}
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('sys-invoke-log-form-data');
		}
		return rawFormData;
	}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('sys-invoke-log-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('sys-invoke-log-form-data-win',this); });
