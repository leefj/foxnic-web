...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('sys-file-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('sys-file-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'sys-file-form-data-win');
			if(area==null) return;
	...
	function renderFormFields() {
		...
		laydate.render({
			...
			trigger:"click",
				...
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("latestVisitTime",value, date, endDate);
			}
		});
	}
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('sys-file-form-data');
		}
		...
		if (hasData) {
			...
			//设置 最后访问 显示复选框勾选
			if(formData["latestVisitTime"]) {
				$("#latestVisitTime").val(fox.dateFormat(formData["latestVisitTime"],"yyyy-MM-dd HH:mm:ss"));
			}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('sys-file-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('sys-file-form-data-win',this); });
