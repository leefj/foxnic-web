...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('sys-job-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('sys-job-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'sys-job-form-data-win');
			if(area==null) return;
			admin.putTempData('sys-job-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("workerId",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			searchField: "className", //请自行调整用于搜索的字段名称
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					if(!data[i]) continue;
					opts.push({data:data[i],name:data[i].className,value:data[i].id,selected:(defaultValues.indexOf(data[i].id)!=-1 || defaultIndexs.indexOf(""+i)!=-1)});
				}
				return opts;
			}
		});
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('sys-job-form-data');
		}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('sys-job-form-data-win',this); });
