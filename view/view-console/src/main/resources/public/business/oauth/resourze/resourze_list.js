...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-oauth/sys-resourze";
	var dataTable=null;
	var sort=null;
	/**
      * 入口函数，初始化
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		value.accessType={ inputType:"radio_box", value: xmSelect.get("#accessType",true).getValue("value"), label:xmSelect.get("#accessType",true).getValue("nameStr") };
	...
	function bindButtonEvent() {
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('sys-resourze-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的')+fox.translate('系统资源')+"!");
            	return;
            }
            //调用批量删除接口
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('sys-resourze-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				//延迟显示加载动画，避免界面闪动
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('sys-resourze-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString="?" + 'id=' + data.id;
		admin.putTempData('sys-resourze-form-data', data);
		var area=admin.getTempData('sys-resourze-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('系统资源');
		...
		var index=admin.popupCenter({
			...
			content: '/business/oauth/resourze/resourze_form.html' + queryString,
			finish: function () {
				refreshTableData();
			}
		});
