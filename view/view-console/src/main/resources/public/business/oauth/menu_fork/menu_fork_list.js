...
function ListPage() {
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('sys-menu-fork-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'菜单功能分版本实现配置'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'菜单功能分版本实现配置'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			if (layEvent === 'edit') { // 修改
				...
				top.layer.confirm(fox.translate('确定删除此'+'菜单功能分版本实现配置'+'吗？'), function (i) {
					top.layer.close(i);
