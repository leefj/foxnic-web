...
function ListPage() {
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('sys-role-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('sys-role-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的')+fox.translate('角色')+"!");
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的')+fox.translate('角色')+fox.translate('吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('sys-role-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				admin.post(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('sys-role-form-data-form-action', "edit",true);
				...
				admin.post(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('sys-role-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
				...
				top.layer.confirm(fox.translate('确定删除此')+fox.translate('角色')+fox.translate('吗？'), function (i) {
					top.layer.close(i);
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('sys-role-form-data-form-action');
		var queryString="";
		...
		admin.popupCenter({
			...
			type: 2,
			id:"sys-role-form-data-win",
			content: '/business/oauth/role/role_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
