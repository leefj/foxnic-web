...
function ListPage() {
		...
		function renderTableInternal() {
			...
			dataTable=fox.renderTable({
				...
				cols: [[
					...
					,{ field: 'allotted', align:"right",fixed:false,  hide:false, sort: true, title: fox.translate('是否分配') }
	...
	function refreshTableData(sortField,sortType) {
		...
		value.allotted={ value: $("#allotted").val()};
	...
	function bindButtonEvent() {
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('pcm-catalog-form-data-form-action', "create",true);
            showEditForm(data);
        };
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('pcm-catalog-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				//延迟显示加载动画，避免界面闪动
				...
				admin.request(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					...
					layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('pcm-catalog-form-data-form-action', "edit",true);
				...
				admin.request(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					...
					layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('pcm-catalog-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						layer.msg(data.message, {icon: 1, time: 1500});
					}
	...
	function showEditForm(data) {
		...
		var index=admin.popupCenter({
			...
			type: 2,
			id:"pcm-catalog-form-data-win",
			content: '/business/pcm/catalog/catalog_form.html' + queryString,
			finish: function () {
				refreshTableData();
			}
		});
		admin.putTempData('pcm-catalog-form-data-popup-index', index);
	};
