...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-pcm/pcm-catalog-attribute";
	var dataTable=null;
	var sort=null;
	/**
      * 入口函数，初始化
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'valid', align:"center",fixed:false,  hide:true, sort: true, title: fox.translate('是否有效'), templet: '#cell-tpl-valid'}
	...
	function refreshTableData(sortField,sortType,reset) {
		var value = {};
		value.field={ inputType:"button",value: $("#field").val()};
		value.versionNo={ inputType:"button",value: $("#versionNo").val()};
		value.dataType={ inputType:"select_box", value: xmSelect.get("#dataType",true).getValue("value"), label:xmSelect.get("#dataType",true).getValue("nameStr") };
	...
	function initSearchFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.list.onSelectBoxChanged && window.pageExt.list.onSelectBoxChanged("dataType",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				...
				case 'batch-del':
					...
					break;
				case 'tool-create-version':
					window.pageExt.list.createVersion && window.pageExt.list.createVersion(selected,obj);
					break;
				case 'tool-apply-version':
					window.pageExt.list.applyVersion && window.pageExt.list.applyVersion(selected,obj);
					break;
				case 'refresh-data':
					refreshTableData();
					break;
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('pcm-catalog-attribute-form-data-form-action', "create",true);
            showEditForm(data);
        };
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('pcm-catalog-attribute-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				admin.post(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('pcm-catalog-attribute-form-data-form-action', "edit",true);
				...
				admin.post(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('pcm-catalog-attribute-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						top.layer.msg(data.message, {icon: 1, time: 1500});
					}
	...
	function showEditForm(data) {
		...
		admin.popupCenter({
			...
			type: 2,
			id:"pcm-catalog-attribute-form-data-win",
			content: '/business/pcm/catalog_attribute/catalog_attribute_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				refreshTableData();
			}
		});
