...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-changes/chs-change";
	var dataTable=null;
	/**
      * 入口函数，初始化
      */
	...
	function bindButtonEvent() {
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('chs-change-form-data-form-action', "create",true);
            showEditForm(data);
        };
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('chs-change-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				//延迟显示加载动画，避免界面闪动
				...
				admin.request(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					...
					if(data.success) {
						admin.putTempData('chs-change-form-data-form-action', "edit",true);
				...
				admin.request(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					...
					layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('chs-change-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						layer.msg(data.message, {icon: 1, time: 1500});
					}
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('chs-change-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString="?" + 'id=' + data.id;
		admin.putTempData('chs-change-form-data', data);
		var area=admin.getTempData('chs-change-form-area');
		...
		var index=admin.popupCenter({
			...
			type: 2,
			id:"chs-change-form-data-win",
			content: '/business/changes/change/change_form.html' + queryString,
			finish: function () {
				refreshTableData();
			}
		});
		admin.putTempData('chs-change-form-data-popup-index', index);
	};
