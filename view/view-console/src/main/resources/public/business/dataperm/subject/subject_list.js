...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-dataperm/dp-subject";
	var dataTable=null;
	/**
      * 入口函数，初始化
      */
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'getter', align:"left",fixed:false,  hide:false, sort: true, title: fox.translate('Subject对象获取类名称') , templet: function (d) { return templet('getter',d.getter,d);}  }
	...
	function bindButtonEvent() {
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('dp-subject-form-data-form-action', "create",true);
            showEditForm(data);
        };
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('dp-subject-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				//延迟显示加载动画，避免界面闪动
				...
				admin.request(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					...
					if(data.success) {
						admin.putTempData('dp-subject-form-data-form-action', "edit",true);
				...
				admin.request(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					...
					layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('dp-subject-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						layer.msg(data.message, {icon: 1, time: 1500});
					}
				...
				top.layer.confirm(fox.translate('确定删除此')+fox.translate('数据权限主体')+fox.translate('吗？'), function (i) {
					top.layer.close(i);
	...
	function showEditForm(data) {
		...
		var title = fox.translate('数据权限主体');
		...
		var index=admin.popupCenter({
			...
			type: 2,
			id:"dp-subject-form-data-win",
			content: '/business/dataperm/subject/subject_form.html' + queryString,
			finish: function () {
				refreshTableData();
			}
		});
		admin.putTempData('dp-subject-form-data-popup-index', index);
	};
