...
function ListPage() {
	...
	function bindButtonEvent() {
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('dp-rule-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的')+fox.translate('数据权限规则')+"!");
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的')+fox.translate('数据权限规则')+fox.translate('吗？'), function (i) {
				top.layer.close(i);
				top.layer.load(2);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('dp-rule-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				//延迟显示加载动画，避免界面闪动
				...
				admin.request(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					...
					if(data.success) {
						admin.putTempData('dp-rule-form-data-form-action', "edit",true);
				...
				admin.request(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					...
					layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('dp-rule-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						layer.msg(data.message, {icon: 1, time: 1500});
					}
				...
				top.layer.confirm(fox.translate('确定删除此')+fox.translate('数据权限规则')+fox.translate('吗？'), function (i) {
					top.layer.close(i);
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('dp-rule-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString="?" + 'id=' + data.id;
		admin.putTempData('dp-rule-form-data', data);
		var area=admin.getTempData('dp-rule-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('数据权限规则');
		...
		var index=admin.popupCenter({
			...
			type: 2,
			id:"dp-rule-form-data-win",
			content: '/business/dataperm/rule/rule_form.html' + queryString,
			finish: function () {
				refreshTableData();
			}
		});
		admin.putTempData('dp-rule-form-data-popup-index', index);
	};
