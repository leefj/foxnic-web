 ...
 layui.use(['form', 'table', 'util', 'settings', 'admin', 'upload','foxnic'], function () {
        ...
        fox.renderTable({
			...
			cols: [[
                ...
                {fixed: 'right', align: 'center', toolbar: '#menu-table-operation', title: fox.translate('操作'), width: 175}
            ]]
        });
        ...
        table.on('tool(menu-table)', function (obj) {
            ...
            if (layEvent === 'edit') { // 修改
                ...
                layer.confirm(fox.translate('确定删除此')+fox.translate('功能菜单')+fox.translate('吗？'), function (i) {
                    layer.close(i);
                    layer.load(2);
        ...
        var showEditModel = function (data) {
            ...
            var title = data ? (fox.translate('修改')+fox.translate('功能菜单')) : (fox.translate('添加')+fox.translate('功能菜单'));
            admin.popupCenter({
                title: title,
                path: '/pages/product/menu/menu_form.html',
                finish: function () {
                    table.reload('menu-table', {});
                }
            });
        ...
        //回车键查询
        $("#menu-search-value").keydown(function(event) {
        	if(event.keyCode ==13){
		  		$("#menu-btn-search").trigger("click");
			}
        });

        // 搜索按钮点击事件
        $('#menu-btn-search').click(function () {
            var field = $('#menu-search-field').val();
            var value = $('#menu-search-value').val();
            table.reload('menu-table', {where: {searchField: field, searchValue: value}});
        });
