 ...
 */
 layui.use(['form', 'table', 'util', 'settings', 'admin', 'upload','foxnic'], function () {
        ...
        fox.renderTable({
			...
			cols: [[
                {type: 'numbers'},
                { field: 'id', sort: true, title: fox.translate('字典ID') } ,
                { field: 'name', sort: true, title: fox.translate('字典名称') } ,
                { field: 'code', sort: true, title: fox.translate('字典代码') } ,
                { field: 'createTime', sort: true, title: fox.translate('创建时间') , templet: function (d) { return util.toDateString(d.createTime); } } ,
                {fixed: 'right', align: 'center', toolbar: '#dict-table-operation', title: fox.translate('操作'), width: 175}
            ]]
        });
        ...
        var showEditModel = function (data) {
            ...
            var title = data ? (fox.translate('修改')+fox.translate('数据字典')) : (fox.translate('添加')+fox.translate('数据字典'));
            admin.popupCenter({
                title: title,
                path: '/pages/product/dict/dict_form.html',
                finish: function () {
                    table.reload('dict-table', {});
                }
            });
        ...
        //回车键查询
        $("#dict-search-value").keydown(function(event) {
        	if(event.keyCode ==13){
		  		$("#dict-btn-search").trigger("click");
			}
        });

        // 搜索按钮点击事件
        $('#dict-btn-search').click(function () {
            var field = $('#dict-search-field').val();
            var value = $('#dict-search-value').val();
            table.reload('dict-table', {where: {searchField: field, searchValue: value}});
        });
