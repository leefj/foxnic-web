...
public class PersonServiceImpl extends SuperService<Person> implements IPersonService {
	...
	public Result deleteByIdLogical(String id) {
		...
		if(id==null) return ErrorDesc.failure().message("id 不允许为 null 。");
		person.setId(id);
		person.setDeleted(dao.getDBTreaty().getTrueValue());
	...
	public Result update(Person person , SaveMode mode) {
		Result r=super.update(person , mode);
		return r;
	}
	 ...
	 * @return 判断结果
	 */
	public Result<Person> checkExists(Person person) {
		//TDOD 此处添加判断段的代码
		//boolean exists=this.checkExists(person, SYS_ROLE.NAME);
		//return exists;
