...
public class ProfileController extends SuperController {
	...
	*/
	...
	@ApiImplicitParams({
		...
		@ApiImplicitParam(name = ProfileVOMeta.LOAD_ORDER , value = "加载的顺序" , required = false , dataTypeClass=Integer.class , example = "0")
	})
	...
	*/
	...
	@NotNull(name = ProfileVOMeta.IDS)
	...
	*/
	...
	@ApiImplicitParams({
		@ApiImplicitParam(name = ProfileVOMeta.ID , value = "主键" , required = true , dataTypeClass=String.class , example = "1"),
	})
	...
	*/
	 ...
	 * 导出 Excel
	 * */
	@SentinelResource(value = ProfileServiceProxy.EXPORT_EXCEL , blockHandlerClass = { SentinelExceptionUtil.class } , blockHandler = SentinelExceptionUtil.HANDLER )
	@RequestMapping(ProfileServiceProxy.EXPORT_EXCEL)
	public void exportExcel(ProfileVO  sample,HttpServletResponse response) throws Exception {
		try{
			//生成 Excel 数据
			ExcelWriter ew=profileService.exportExcel(sample);
			//下载
	 ...
	 * 导出 Excel 模板
	 * */
	@SentinelResource(value = ProfileServiceProxy.EXPORT_EXCEL_TEMPLATE , blockHandlerClass = { SentinelExceptionUtil.class } , blockHandler = SentinelExceptionUtil.HANDLER )
	@RequestMapping(ProfileServiceProxy.EXPORT_EXCEL_TEMPLATE)
	public void exportExcelTemplate(HttpServletResponse response) throws Exception {
		try{
			//生成 Excel 模版
			ExcelWriter ew=profileService.exportExcelTemplate();
			//下载
			DownloadUtil.writeToOutput(response, ew.getWorkBook(), ew.getWorkBookName());
		} catch (Exception e) {
			DownloadUtil.writeDownloadError(response,e);
	...
	@SentinelResource(value = ProfileServiceProxy.IMPORT_EXCEL , blockHandlerClass = { SentinelExceptionUtil.class } , blockHandler = SentinelExceptionUtil.HANDLER )
	@RequestMapping(ProfileServiceProxy.IMPORT_EXCEL)
	public Result importExcel(MultipartHttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		List<ValidateResult> errors=profileService.importExcel(input,0,true);
